...
func copyContext(in *lib.Ctx) *lib.Ctx {
	out := lib.Ctx{
		...
		MaxParallelAPIReviews: in.MaxParallelAPIReviews,
...
func TestInit(t *testing.T) {
	...
	var testCases = []struct {
		...
		{
			"Setting max parallel GitHub API reviews",
			map[string]string{"GHA_MAX_PARALLEL_API_REVIEWS": "16"},
			dynamicSetFields(
				t,
				copyContext(&defaultContext),
				map[string]interface{}{"MaxParallelAPIReviews": 16},
			),
		},
		{
			"Setting max parallel SHAs",
